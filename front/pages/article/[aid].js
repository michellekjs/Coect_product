            ...
            <div style={styles.container}>
                <div style={styles.article}>
                    <div>
                        ...
                        <div style={{ fontSize: 20, color: colors.primary, fontWeight: 'bold' }}>{categories[article.category-1]}</div>
                        ...
                        <div>
                            ...
                            
                            {
                                article.summaries.map((summary,i) => (
                                    summary.text ? 
                                    <TextUnit key={i} subject={summary.subject} start={summary.start} end={summary.end}>{summary.text}</TextUnit> : 
                                    <div key={i} style={{ textAlign: 'center', marginTop: 20, marginBottom: 20 }}>
                                        <img src={`/imgs/figures/${summary.image}`} style={{ width: 600 }} alt="영상 캡쳐 이미지"></img>
                                    </div>
                                ))
                            }
                        </div>
                ...
                <div style={styles.recommended}>
                    ...
                    
                    {
                        relatedArticlesByKeywords.map(({keyword, articles}) => 
                            articles.length ? [
                                <h4 key={keyword} style={styles.h4}>{keyword}</h4>,
